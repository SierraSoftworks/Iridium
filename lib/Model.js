...
var Model = (function () {
     ...
     * @constructor
     */
    function Model(core, collection, schema, options) {
        if (options === void 0) { options = {}; }
        // Allow instantiation doing `require('iridium').Model(db, 'collection', {})`
        if (!(this instanceof Model))
            return new Model(core, collection, schema, options);
        options = options || {};
        _.defaults(options, {
            hooks: {},
            transforms: [
        ...
        this._Instance = new ModelSpecificInstance(this);
     ...
     * @constructor
     */
    Model.prototype.Instance = function (document, isNew, isPartial) {
    ...
    Model.prototype.find = function (conditions, options, callback) {
        ...
        return Promise.resolve().then(function () {
            ...
            if (!_.isPlainObject(conditions))
                conditions = $this.helpers.selectOneDownstream(conditions);
    ...
    Model.prototype.findOne = function () {
        ...
        return Promise.resolve().bind(this).then(function () {
            ...
            if (!document)
                return null;
            return $this.handlers.documentsReceived(conditions, [document], $this.helpers.wrapDocument, options).then(function (documents) {
                return documents[0];
            });
        }).nodeify(callback);
    };
    ...
    Model.prototype.insert = function (objs) {
        ...
        return Promise.resolve().then(function () {
            ...
            if (options.upsert)
                return $this.handlers.creatingDocuments(objects).map(function (object) {
            ...
            return $this.handlers.documentsReceived(null, inserted, $this.helpers.wrapDocument, { wrap: options.wrap, cache: options.cache });
        }).then(function (results) {
    ...
    Model.prototype.ensureIndex = function (specification, options, callback) {
        ...
        var $this = this;
        return new Promise(function (resolve, reject) {
            $this.collection.ensureIndex(specification, options, function (err, name) {
                if (err)
                    return reject(err);
                return resolve(name);
            });
...
var ModelSpecificInstance = (function () {
    function ModelSpecificInstance(model) {
        this.Constructor = function (document, isNew, isPartial) {
            instance.Instance.call(this, model, document, isNew, isPartial);
        };
        util.inherits(this.Constructor, instance.Instance);
...
var ModelHelpers = (function () {
     ...
     * @returns {instance.IInstance<TSchema>} An instance which wraps this document
     */
...
var ModelHandlers = (function () {
    ...
    ModelHandlers.prototype.documentsReceived = function (conditions, results, wrapper, options) {
        ...
        return Promise.resolve(results).map(function (target) {
            return Promise.resolve().then(function () {
                ...
                if (this.model.hooks.retrieved)
                    return this.model.hooks.retrieved(target);
            }).then(function () {
                ...
                if (this.model.hooks.ready)
                    return Promise.resolve(this.model.hooks.ready(wrapped)).then(function () { return wrapped; });
                return wrapped;
            });
        });
    };
    ModelHandlers.prototype.creatingDocuments = function (documents) {
        return Promise.resolve(documents).map(function (document) {
            return Promise.resolve().then(function () {
                if (this.model.hooks.retrieved)
                    return this.model.hooks.creating(document);
            }).then(function () {
                var validation = this.model.helpers.validate(document);
    ...
    ModelHandlers.prototype.savingDocument = function (instance, changes) {
        return Promise.resolve().then(function () {
            if (this.model.hooks.saving)
                return this.model.hooks.saving(instance, changes);
...
var ModelCache = (function () {
    ...
    ModelCache.prototype.set = function (conditions, value) {
        if (!this.model.cache.valid(conditions))
    ...
    ModelCache.prototype.get = function (conditions) {
        ...
        return this.model.core.cache.get(this.model.cache.buildKey(conditions));
    };
