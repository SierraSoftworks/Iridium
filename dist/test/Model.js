...
describe("Model", () => {
    ...
    describe("constructor", () => {
        function createInstanceImplementation(properties) {
    ...
    describe("find()", () => {
        ...
        it("should return a cursor object", () => {
            chai.expect(model.find()).to.be.an.instanceof(Cursor_1.Cursor);
        });
        describe("each()", () => {
            ...
            it("should resolve the promise after all handlers have been dispatched", () => {
                ...
                return chai.expect(model.find().forEach((instance) => {
                    count++;
                }).then(() => chai.expect(count).to.not.equal(5)).then(() => Promise_1.Delay(10)).then(() => count)).to.eventually.equal(5);
            });
            ...
            it("should support using callbacks instead of promises", (done) => {
                ...
                model.find().forEach(i => count++, (err) => {
                    if (err)
                        return done(err);
                    Promise_1.Delay(10).then(() => chai.expect(count).to.eql(5)).then(() => done());
                });
            });
        });
        describe("map()", () => {
        ...
        describe("rewind()", () => {
            it("should return a new cursor", () => {
                chai.expect(model.find().rewind()).to.be.an.instanceof(Cursor_1.Cursor);
            });
        ...
        describe("limit()", () => {
            it("should return a new cursor", () => {
                chai.expect(model.find().limit(1)).to.be.instanceof(Cursor_1.Cursor);
            });
        ...
        describe("skip()", () => {
            it("should return a new cursor", () => {
                chai.expect(model.find().skip(1)).to.be.instanceof(Cursor_1.Cursor);
            });
            it("which should impose the limit", () => {
        ...
        describe("sort()", () => {
            it("should return a new cursor", () => {
                chai.expect(model.find().sort({ answer: 1 })).to.be.instanceof(Cursor_1.Cursor);
            });
        ...
        describe("readFrom()", () => {
            it("should return a new cursor", () => {
                chai.expect(model.find().readFrom("secondaryPreferred")).to.be.instanceof(Cursor_1.Cursor);
            });
        });
    });
    describe("count()", () => {
    ...
    describe("ensureIndexes()", () => {
        ...
        before(() => {
            ...
            model = new Iridium.Model(core, Test);
            return core.connect().then(() => model.remove()).then(() => model.insert([
                { answer: 10 },
                { answer: 11 },
                { answer: 12 },
